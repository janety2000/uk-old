...
# UK (myjobmag.co.uk):        /jobs/page/{n}
# Override via SITE_LIST_PAGE_PATH env var for other country sites if the
# pattern differs there too — check the pagination links at the bottom of
# any listing page (e.g. https://www.myjobmag.co.uk/jobs) to confirm.
LIST_PAGE_PATH = os.environ.get("SITE_LIST_PAGE_PATH", "/page/{page}")

START_PAGE   = int(os.environ.get("START_PAGE", "139"))   # scrape starts here on a *fresh* run (no saved state)
END_PAGE     = int(os.environ.get("END_PAGE", "1"))         # and goes down to (and including) here, eventually

# A short site key so different country runs (NG, UK, ...) keep separate
# progress/dedup files in the same repo instead of clobbering each other.
SITE_KEY = os.environ.get("SITE_KEY", "ng")
...
REQUEST_TIMEOUT = 20
# Nigeria keeps its original filenames for backward compatibility with the
# processed.csv / current_page.txt already committed to the repo from prior
# runs. Any other SITE_KEY gets its own suffixed files so multiple country
# scrapers can share the same repo without clobbering each other's state.
if SITE_KEY == "ng":
    PROCESSED_IDS_FILE = "processed.csv"
    PAGE_STATE_FILE = "current_page.txt"
else:
    PROCESSED_IDS_FILE = f"processed_{SITE_KEY}.csv"
    PAGE_STATE_FILE = f"current_page_{SITE_KEY}.txt"
...
def scrape_job_list_page(page_num: int) -> list:
    url = BASE_URL + LIST_PAGE_PATH.format(page=page_num)
...
def _is_expired(soup) -> bool:
    """
    Expired listings on this network show an '#expired-suggestion-jobs'
    block above the job content (e.g. "Oops! It seems this job from X has
    expired"). The rest of #printable still parses fine, so without this
    check the scraper would happily re-post stale, no-longer-open jobs.
    """
    return soup.select_one("#expired-suggestion-jobs") is not None
...
def scrape_job_details(job_url: str) -> list:
    ...
    if _is_expired(soup):
        logger.info(f"⏭ SKIP — listing marked expired on source site: {job_url}")
        return []
...
def run():
    ...
    logger.info(f"🌍 Site: {SITE_KEY} — {BASE_URL}{LIST_PAGE_PATH.format(page='<n>')}")
